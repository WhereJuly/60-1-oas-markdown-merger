...
describe('CLI Application Usage', () => {
    ...
    it.only('Should successfully merge OAS file', () => {
        const expectedMarkdown = '../tests/foundation/.ancillary/fixtures/markdown/simple.md';

        const outputFile = `${tempFolder}/output.json`;
        const output = execSync(`npx oas-markdown-merger --input ../tests/foundation/.ancillary/fixtures/definitions/petstore.oas.json --output ${outputFile}`).toString();
    ...
    it('Should handle errors gracefully', async () => {
        try {
            exec('npx oas-markdown-merger --input input.json --output output.json');
